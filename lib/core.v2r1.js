...
Orb.Observer =  function(pos){
  var rad=Math.PI/180;
  var a = 6377.39715500; // earth radius
  var e2 = 0.006674372230614;
  var n = a/(Math.sqrt(1-e2*Math.cos(pos.latitude*rad)))
  var ToRectangular = function(pos){
      return {
        x: (n+pos.altitude)*Math.cos(pos.latitude*rad)*Math.cos(pos.longitude*rad),
        y: (n+pos.altitude)*Math.cos(pos.latitude*rad)*Math.sin(pos.longitude*rad),
        z: (n*(1-e2)+pos.altitude)*Math.sin(pos.latitude*rad)
      }
  }
  return {
    position:{
      ...
      rectangular: function(){
        return ToRectangular(pos)
      }
    } 
  }// end of return Orb.Observer
} // end of Orb.Observer

Orb.Observation = function(param){
